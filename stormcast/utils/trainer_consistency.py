...
from physicsnemo.experimental.metrics.diffusion import ConsistencyDistillationLoss
...
from .ema import ExponentialMovingAverage, ema_decay_schedule
...
logger = PythonLogger("train_consistency")
...
def consistency_training_loop(cfg):
    ...
    # Build student (ConsistencyPrecond)
    student = get_preconditioned_architecture(name="consistency", **arch_kwargs)
    ...
    ema = ExponentialMovingAverage(student, decay=cfg.training.ema_decay_init)
    ema_net = get_preconditioned_architecture(name="consistency", **arch_kwargs)
    ...
    loss_fn = ConsistencyDistillationLoss(
        ...
        N_0=cfg.training.N_0,
        N_total=cfg.training.N_total,
        total_train_steps=total_train_steps,
        huber_c=cfg.training.huber_c,
    )
    if cfg.training.compile_model:
    ...
    if total_steps == 0:
        ...
        if init_weights is None or init_weights == "":
            logger0.info("Starting consistency distillation from teacher-initialized weights...")
        else:
        ...
        if total_steps % cfg.training.validation_freq == 0:
            ...
            with torch.no_grad():
                ...
                with torch.autocast("cuda", dtype=amp_dtype, enabled=enable_amp):
                    ...
                    # 1-step generation via consistency model
                    output_images = consistency_model_forward(
                        student,
                        condition,
                        state[1].shape,
                        sigma_max=cfg.model.sigma_max,
                    ...
                    # Compute validation loss (same CD loss for monitoring)
                    valid_loss = loss_fn(
                        student=student,
                        teacher=teacher,
                        ema_student=ema_net,
