...
"""Anchored Stochastic-Interpolant Bridge training loop.

Cousin of :mod:`trainer_flowcast`: shares all the DDP / EMA / checkpoint /
validation-plot scaffolding, swaps the I-CFM core for a bridge whose prior
endpoint is the frozen regression mean mu_{t+1}. Architecture, conditioning,
hybrid-loss knobs (per-channel weights + log-PSD term on qpepre) all stay
identical so the two methods are directly comparable.

Key differences vs. ``flowcast_training_loop``:
    * ``subtract_regression=False`` in build_network_condition_and_target,
      so the training target is the raw next-state ``M_{t+1}`` (the bridge
      consumes mu separately as the prior endpoint).
    * Loss is :class:`BridgeMatchingLoss` (raw-space, no sigma_data scaling).
    * Validation sampling uses :func:`bridge_model_forward`, which already
      returns ``M_pred`` directly -- no need to add mu back on top.
"""
...
from .bridge_loss import BridgeMatchingLoss
...
logger = PythonLogger("train_bridge")
...
def bridge_training_loop(cfg):
    """Main training loop for the anchored Stochastic-Interpolant Bridge."""
    ...
    # 'regression' MUST be in condition_list -- the bridge needs mu both as
    # the prior endpoint AND as a conditioning channel.
    if "regression" not in condition_list:
        raise ValueError(
            "Bridge training requires 'regression' in model.diffusion_conditions "
            "so mu_{t+1} is available as the prior endpoint."
        )
    ...
    student = get_preconditioned_architecture(name="bridge", **arch_kwargs)
    ...
    ema_net = get_preconditioned_architecture(name="bridge", **arch_kwargs)
    ...
    prior_channel_std = getattr(cfg.training, "prior_channel_std", None)
    if prior_channel_std is not None:
        prior_channel_std = list(prior_channel_std)

    loss_fn = BridgeMatchingLoss(
        sigma_prior=float(cfg.training.sigma_prior),
        sigma_max=float(cfg.training.sigma_max),
        schedule=str(cfg.training.schedule),
        coupling=str(cfg.training.coupling),
    ...
    if total_steps == 0:
        ...
        if init_weights is None or init_weights == "":
            logger0.info("Starting Bridge training from random initialization...")
        else:
        ...
        for accum_idx in range(num_accumulation_rounds):
            ...
            with sync_ctx:
                with torch.autocast("cuda", dtype=amp_dtype, enabled=enable_amp):
                    # Bridge: target is RAW M_{t+1}; mu is returned separately.
                    (condition, target, mu) = build_network_condition_and_target(
                        background,
                        state,
                        invariant_tensor,
        ...
        if total_steps % cfg.training.validation_freq == 0:
            ...
            with torch.no_grad():
                ...
                with torch.autocast("cuda", dtype=amp_dtype, enabled=enable_amp):
                    ...
                    # Bridge sampler returns M_pred directly -- no + mu.
                    output_images = bridge_model_forward(
                        ema_net,
                        condition,
                        mu,
