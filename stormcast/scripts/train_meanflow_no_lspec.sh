#!/bin/bash
# Launch MeanFlow training WITHOUT the spectral (radial log-PSD) loss term.
# Everything else is identical to train_meanflow.sh so this is a clean A/B on
# the spectral regularizer alone: spectral_weight=0 and an empty
# spectral_channels list make MeanFlowLoss return a zero spectral term (see
# stormcast/utils/meanflow_loss.py and trainer_meanflow.py:288-292).
# Paths below assume the local workstation layout (see CLAUDE.md §1).
...
stormcast_train="$(dirname "$0")/../train_meanflow.py"
config="--config-name meanflow"
experiment_name="meanflow_no_lspec"
training_output_dir="$(pwd)/StormCast_meanflow_no_lspec"
run_id="0"
...
# --- Training parameters (kept identical to FlowCast for a clean A/B) ---
batch_size=16
lr=5E-4
weight_decay=1E-4
...
adaptive_p=1.0        # adaptive loss weight exponent (0 disables)
adaptive_eps=1.0E-3
sigma_data=0.5        # match the EDM teacher / FlowCast student
time_scale=1000.0
