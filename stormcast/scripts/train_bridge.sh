#!/bin/bash
# Launch anchored Stochastic-Interpolant Bridge training on Taiwan RWRF.
# Direction 1 of flowcast_improvement.md. Mirrors train_flowcast.sh; only the
# loss / sampler differ. Override the dataset / regression checkpoint paths
# below to point at your cleaned zarr + frozen regression checkpoint.
...
# --- Bridge parameters ---
# sigma_prior: std of the additive Gaussian on the prior endpoint
#   (x_0 = mu + sigma_prior * eps). Small (~0.05) keeps the start close to
#   mu so the bridge is geometrically short.
# sigma_max:  peak of the bridge's interior noise term gamma(t)*z. Roughly
#   the magnitude of the residual after standardization. Set to 0 to ablate
#   the stochastic interpolant (collapses to deterministic residual head).
...
spectral_weight=0.1
# Optional per-channel prior std multipliers; Direction 2 ablation.
# E.g. "[1.0,1.0,1.0,2.0]" boosts the qpepre prior std relative to the others.
