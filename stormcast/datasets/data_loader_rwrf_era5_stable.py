...
class Dataset(StormCastDataset):
    ...
    def _collect_overlapping_valid_datetimes(self) -> List[datetime]:
        """Gather timesteps where both LowRes and HighRes mark the data as valid."""
        if not self.ds_LowRes or not self.ds_HighRes:
            return []
        overlapped = []
        for idx in range(self.n_zarrs):
            low_ds = self.ds_LowRes[idx]
            high_ds = self.ds_HighRes[idx]
            low_times = np.asarray(low_ds.time.values)
            high_times = np.asarray(high_ds.time.values)
            if low_times.shape != high_times.shape or not np.array_equal(low_times, high_times):
                raise ValueError(
                    f"Time coordinates mismatch between {self.LowRes_paths[idx]} and {self.HighRes_paths[idx]}"
                )
            low_mask = self._load_valid_mask(low_ds, self.LowRes_paths[idx])
            high_mask = self._load_valid_mask(high_ds, self.HighRes_paths[idx])
            mask = np.logical_and(low_mask, high_mask)
            if not np.any(mask):
                continue
            overlapped.extend(low_times[mask])
        unique_datetimes = sorted({self.to_datetime(ts) for ts in overlapped})
    ...
    def compute_total_samples(self):
        ...
        overlapped_datetimes = self._collect_overlapping_valid_datetimes()
        if not overlapped_datetimes:
            self.logger0.warning("No overlapping valid timestamps found between LowRes and HighRes")
            self.valid_samples = []
            return 0

        in_range = [
            ts
            for ts in overlapped_datetimes
            if first_sample <= ts <= last_sample
        ]
        in_range.sort()
        available = set(in_range)

        self.valid_samples = []
        for ts in in_range:
            target = ts + timedelta(hours=self.dt)
            if target > last_sample:
                continue
            if target not in available:
                continue
            self.valid_samples.append(ts)

        self.logger0.info(
            "Total overlapped datetimes in range: {} (dt={} ⇒ usable samples={})".format(
                len(in_range),
                self.dt,
