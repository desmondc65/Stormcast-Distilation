#!/usr/bin/env bash
# Upload the finished MeanFlow ABLATION SUITE results from the zettabyte worker
# up to Azure Blob, so they can be pulled back to a workstation with the
# companion ../../zettabyte/sync_results_from_azure.sh.
#
# Run this ON the zettabyte worker (azcopy is preinstalled there) AFTER
# train_meanflow_ablations.sh has finished. It is a single idempotent pass:
# `azcopy sync` only transfers files whose size/mtime changed, so it is safe to
# re-run (e.g. after adding more ablations) without re-uploading everything.
#
# What it uploads: the whole ablation base tree written by the suite
#   <ABLATION_BASE>/<exp>/run_0/{checkpoints_meanflow,*.csv,images/,hydra/}
# i.e. the single MeanFlowPrecond.0.18000.mdlus (+ ema_state.pt) per ablation
# plus every loss/RMSE/PS1D CSV and validation heatmap. (Source default matches
# ABLATION_BASE in ../../experiment_scripts/meanflow_ablations/_common.sh.)
#
...
#
# Usage:
#   ./upload_meanflow_ablations.sh [SRC] [REMOTE_REL]
#   SRC          local dir on the worker to mirror   (default: ABLATION_BASE below)
...
# --- Defaults: match the ablation suite's ABLATION_BASE ----------------------
DEFAULT_SRC="/data/exp_3_train_2_5_yrs_val_1yr_tp1/meanflow_ablations_cleaned_4_27_2026"
...
if [[ ! -d "$SRC" ]]; then
    ...
    echo "        (run train_meanflow_ablations.sh first, or pass SRC explicitly)" >&2
    exit 1
fi

echo "[upload_ablations] config:"
echo "    SRC          = $SRC"
echo "    REMOTE       = $REMOTE_REL"
echo "    SYNC_FLAGS   = ${SYNC_FLAGS:-<none>}"
echo "[upload_ablations] uploading $SRC -> ${REMOTE_REL}/ ..."
...
if [[ $rc -ne 0 ]]; then
    echo "[upload_ablations] azcopy sync FAILED (exit $rc)" >&2
    exit $rc
fi

echo "[upload_ablations] done. Verify the remote tree with:"
echo "    azcopy list \"${SAS_BASE}/${REMOTE_REL}/?\$SAS_QUERY\""
echo "[upload_ablations] pull it back to a workstation with:"
