#!/bin/bash
# Train the StormCast MeanFlow student WITHOUT the spectral (radial log-PSD)
# loss term on zettabyte cloud, 7x H100, on top of the cleaned dataset and the
# regression checkpoint trained at step 8000.
#
# This is the no-spectral A/B partner of the production MeanFlow leg
# (train_meanflow.sh / the canonical meanflow_zettabyte run). EVERYTHING ELSE
# -- dataset, regression mean, AdamW/cosine, channel order, sigma_data,
# mf_ratio, channel_weights, sample budget -- is kept identical, so toggling
# the spectral term is the only difference. The spectral penalty is disabled by
# passing an empty spectral_channels list and spectral_weight=0; MeanFlowLoss
# then returns a zero spectral term (see ../utils/meanflow_loss.py:214 and
# ../utils/trainer_meanflow.py:288-292).
#
# Budget: ~2.0M samples (batch_size=112 x total_train_steps=18000), matching the
# MeanFlow ablation suite. checkpoint_freq == total_train_steps, so a SINGLE
# final checkpoint is written at the end of training (plus ema_state.pt).
#
...
mkdir -p "${log_dir}"
log_file="${log_dir}/train_meanflow_no_lspec_$(date +%Y%m%d_%H%M%S).log"
...
experiment_name="meanflow_no_lspec_zettabyte_cleaned_4_27_2026"
training_output_dir="/data/exp_3_train_2_5_yrs_val_1yr_tp1/meanflow_no_lspec_zettabyte_v1_cleaned_4_27_2026"
run_id="0"

# --- Logging parameters ---
print_progress_freq=25
checkpoint_freq=18000      # == total_train_steps -> single final checkpoint
validation_freq=250
num_data_workers=4
...
total_train_steps=18000    # 112 x 18000 ~= 2.0M samples
clip_grad_norm=1.0
...
time_scale=1000.0          # scales r in [0,1] into SongUNet's positional-embed range

# --- Sampler (validation only) ---
valid_num_steps=2          # average-velocity segments (NFE); 1 = one-step

# --- Hybrid loss (per-channel β only -- spectral term DISABLED) ---
# Order of channel_weights MUST match kept_HighRes_channels below: u10, v10, t2m, qpepre.
channel_weights="[1.0,1.0,1.0,2.0]"
spectral_channels="[]"     # no channels -> no radial log-PSD penalty
spectral_weight=0.0        # belt-and-suspenders: zero weight also disables it
...
# scripts so the resulting student is drop-in compatible.
